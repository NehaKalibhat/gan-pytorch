...
class GAN(nn.Module):
    def __init__(self, hidden_size, latent_size, image_size = 28, input_channels = 3):
        ...
        self.D = nn.Sequential(
            ...
            nn.ReLU(True),
            nn.Conv2d(hidden_size * 2, hidden_size * 4, 5, 1, 0),
            nn.BatchNorm2d(hidden_size * 4),
            nn.ReLU(True),
            nn.Conv2d(hidden_size * 4, 1, 3, 1, 0),
            nn.Sigmoid()
        )

        self.G = nn.Sequential(
            nn.ConvTranspose2d(latent_size, hidden_size * 4, 3, 1, 0),
            nn.BatchNorm2d(hidden_size * 4),
            nn.ReLU(True),
            nn.ConvTranspose2d(hidden_size * 4, hidden_size * 2, 5, 1, 0),
            nn.BatchNorm2d(hidden_size * 2),
            nn.ReLU(True),
...
#                 gen = self.denorm(gen)
    ...
    def load(self, model_path, prune_gen = True, prune_disc = True):
        ...
        for name in model_state['gan']:
            if ("G" in name and prune_gen) or ("D" in name and prune_disc):
    ...
    def iterative_prune(self, init_state, 
        ...
        for pruning_iter in range(0, number_of_iterations):
            ...
            self.__init__(image_size = image_size * image_size, hidden_size = hidden_size, latent_size = latent_size)
            self = self.cuda()
    ...
    def train(self, prune, init_state, init_with_old, prune_gen = True, prune_disc = True):
...
model = GAN(image_size = image_size * image_size * 3, hidden_size = hidden_size, latent_size = latent_size)
